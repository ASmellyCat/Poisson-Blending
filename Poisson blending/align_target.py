...
def align_target(source_image, target_image):
    ...
    while True:
        ...
        yind2 = np.clip(np.arange(ymin, ymax) - int(y.mean()) + int(ty), 0, target_copy.shape[0] - 1).astype(np.int32)
        yind = yind2 + int(y.mean()) - int(ty)
        xind2 = np.clip(np.arange(xmin, xmax) - int(x.mean()) + int(tx), 0, target_copy.shape[1] - 1).astype(np.int32)
        xind = xind2 + int(x.mean()) - int(tx)

        y = np.clip(y - int(y.mean()) + int(ty), 0, target_copy.shape[0] - 1).astype(np.int32)
        x = np.clip(x - int(x.mean()) + int(tx), 0, target_copy.shape[1] - 1).astype(np.int32)
